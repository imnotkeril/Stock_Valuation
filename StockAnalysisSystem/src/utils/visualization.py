...
class FinancialVisualizer:
    ...
    def plot_financial_ratios(self,
        ...
        try:
            ...
            if any(val is not None for val in benchmark_values):
                fig.add_trace(
                    go.Bar(
                        x=ratio_names,
                        y=benchmark_values,
            ...
            fig.update_layout(
                ...
                font=dict(
                    color=self.text_color,
                    family=self.settings.get('font_family', 'Arial, sans-serif')
                ),
                legend=dict(
                    orientation="h",
                    yanchor="bottom",
                ...
                barmode='group',
                height=height or self.settings.get('height', 600),
                width=width or self.settings.get('width', 800),
            ...
            for model_name, model_data in models.items():
                if model_data.get('score') is not None:
                    ...
                    if model_name == 'altman_z_score':
                        ...
                        # Define zones
                        zones = [
                            {'range': [0, thresholds['grey']], 'color': self.colors['danger']},
                            {'range': [thresholds['grey'], thresholds['safe']], 'color': self.colors['warning']},
                            {'range': [thresholds['safe'], max_val], 'color': self.colors['success']}
                        ]
                        ...
                        zones = [
                            {'range': [0, threshold], 'color': self.colors['danger']},
                            {'range': [threshold, max_val], 'color': self.colors['success']}
                        ]
                        ...
                        if probability is not None:
                            ...
                            zones = [
                                {'range': [0, threshold], 'color': self.colors['success']},
                                {'range': [threshold, max_val], 'color': self.colors['danger']}
                            ]
                            ...
                            zones = [
                                {'range': [-10, 0], 'color': self.colors['success']},
                                {'range': [0, max_val], 'color': self.colors['danger']}
                            ]
                        ...
                        zones = [
                            {'range': [0, 5], 'color': self.colors['danger']},
                            {'range': [5, 7.5], 'color': self.colors['warning']},
                            {'range': [7.5, max_val], 'color': self.colors['success']}
                        ]

                        title = {'text': model_data.get('description', 'Risk Score')}

                    # Determine value to display
                    display_value = model_data.get('score')
                    if model_name == 'zmijewski' and probability is not None:
                        display_value = probability
    ...
    def plot_dcf_sensitivity(self,
        ...
        try:
            ...
            sensitivity = dcf_data.get('sensitivity', {})

            # Validate sensitivity data structure
            if not isinstance(sensitivity, dict):
                return self._empty_chart("Invalid DCF sensitivity data format")

            # Extract key sensitivity components with safe defaults
            growth_rates = sensitivity.get('growth_rates', [])
            discount_rates = sensitivity.get('discount_rates', [])
            values = sensitivity.get('values', [])

            # Validate data dimensions
            if not (growth_rates and discount_rates and values):
                return self._empty_chart("Incomplete DCF sensitivity data")

            # Create figure
            fig = go.Figure()

            # Get current price, default to None if not available
            ...
            if current_price and current_price > 0:
                ...
                heatmap_values = [
                    [(val / current_price - 1) * 100 for val in row]
                    for row in values
                ]
                ...
                text_matrix = [
                    [
                        f"Growth: {growth_rates[i]}%<br>Discount: {discount_rates[j]}%<br>"
                        f"Value: ${values[i][j]:.2f}<br>"
                        f"Diff from current: {heatmap_values[i][j]:.2f}%"
                ...
                # If no current price, use absolute values
                text_matrix = [
                    [
                        f"Growth: {growth_rates[i]}%<br>"
                        f"Discount: {discount_rates[j]}%<br>"
                        f"Value: ${values[i][j]:.2f}"
                        for j in range(len(discount_rates))
                    ]
                    for i in range(len(growth_rates))
                ]
            ...
            # Mark base case if available
            base_case = dcf_data.get('base_case', {})
            base_growth = base_case.get('growth_rate')
            base_discount = base_case.get('discount_rate')

            if base_growth is not None and base_discount is not None:
                # Find closest values in arrays
                growth_idx = min(
                    range(len(growth_rates)),
                    key=lambda i: abs(growth_rates[i] - base_growth)
                )
                discount_idx = min(
                    range(len(discount_rates)),
                    key=lambda i: abs(discount_rates[i] - base_discount)
                )
